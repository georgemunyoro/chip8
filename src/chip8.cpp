...
Chip8::Chip8(char *romFilePath) {
  ...
  SP = 0;
  delayTimer = 0;
  for (int i = 0; i < 16; ++i) keyboard[i] = false;
}
...
void Chip8::loadROMFileFromPath(char *romFilePath) {
  ...
  this->PC = 0x200;
  while (romFile) memory[PC++] = romFile.get();
  this->PC = 0x200;
        ...
        if (diff.count() >= 0.001666f) {
          switch (opcode & 0xF000) {
              ...
              if (opcode == 0x00EE) {
                ...
                if (opcode == 0x00E0) {
                  memcpy(display[0].bits, display[1].bits,
                         32 * sizeof(uint64_t));
                  for (int i = 0; i < 32; ++i) display[1].bits[i] = 0ull;
                }
            ...
            case 0xD000: {
              ...
              memcpy(display[0].bits, display[1].bits, 32 * sizeof(uint64_t));
              ...
              for (int i = 0; i < n; ++i) {
                ...
                if (valueToXOR & display[1].bits[(Y + i) % 32]) V[0xF] = 1;
        ...
        if (now - interval >= 17) {
          ...
          for (int displayRow = 0; displayRow < 32; ++displayRow) {
            for (int displayCol = 0; displayCol < 64; ++displayCol) {
              bool isPixelLit =
                  ((display[1].bits[displayRow] | display[0].bits[displayRow]) &
                   (1ull << (63 - displayCol))) != 0;

              if (!isPixelLit) continue;
